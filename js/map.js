...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    languageCounts.forEach(([language, count]) => {
        ...
        if (coord) {
            ...
            mapGroup.append("circle")
                .attr("cx", x)
                .attr("cy", y)
                .attr("r", Math.sqrt(count) / 150 + 1.5) // 按比例缩小点的大小
                .attr("fill", "yellow")
                .on("mouseover", function () {
                    d3.select(this).attr("fill", "white"); // 高亮当前点
                    tooltip.style("visibility", "visible")
                        .text(`${language}: ${count} comments`);

                // 高亮使用该语言的国家板块
                mapGroup.selectAll("path")
                    .filter(d => d.properties && d.properties.name) // 确保有国家名称
                    .each(function (d) {
                        const countryName = d.properties.name.toLowerCase();
                        // 自定义逻辑：根据国家与语言的匹配关系高亮
                        const isHighlighted = matchCountryLanguage(language, countryName); 
                        if (isHighlighted) {
                            d3.select(this)
                                .attr("fill", "orange"); // 高亮匹配的国家
                        }
                    });
                })
            ...
            .on("mouseout", function () {
                d3.select(this).attr("fill", "yellow"); // 恢复点的默认颜色
    ...
    // 自定义函数：匹配国家与语言
    function matchCountryLanguage(language, countryName) {
    const languageCountryMap = {
        ...
        schinese: ["china"],
        koreana: ["south korea"],
        spanish: ["spain", "mexico", "argentina", "colombia", "chile", "venezuela", "peru"],
        russian: ["russia", "kazakhstan", "belarus", "ukraine"],
        turkish: ["turkey"],
        ...
        latam: ["mexico", "argentina", "colombia", "chile", "venezuela", "peru"],
        swedish: ["sweden"],
        vietnamese: ["vietnam"],
    ...
    const countries = languageCountryMap[language.toLowerCase()] || [];
